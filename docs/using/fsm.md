...
description: 'Создание диалогов и пошаговых сценариев с помощью FSM (Finite State Machine).'
---
...
$bot->onBotCommand('reg', '/reg')
    ->func(function(ZG $tg) {
        $tg->reply("Давайте знакомиться! Как вас зовут?");
...
$bot->onState('wait_name')
    ->func(function(ZG $tg, string $name) {
        ...
        $tg->reply("Приятно познакомиться, {$name}. Сколько вам лет?");
...
$bot->onState('wait_age')
    ->func(function(ZG $tg, string $age) {
        // Простая валидация
        if (!is_numeric($age)) {
            $tg->reply("Пожалуйста, введите возраст цифрами.");
            return; // Не меняем шаг, пользователь останется в wait_age
        }

        $tg->session(['age' => $age]);
        
        $tg->reply("Отлично! Из какого вы города?");
        $tg->step('wait_city');
    });
```
